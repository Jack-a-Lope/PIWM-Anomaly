...
# ----------------------------------------------------------------------
def render_frame(env, new_api, gray=GRAYSCALE):
    ...
    if gray:
        ...
        return cv2.resize(img, (FRAME_W, FRAME_H), interpolation=cv2.INTER_AREA)
    return cv2.resize(img, (FRAME_W, FRAME_H), interpolation=cv2.INTER_AREA)
...
def rollout(env, new_api, seed, policy, cfg):
    ...
    images = [render_frame(env, new_api, cfg["gray"])]
    ...
    for _ in range(cfg["max_steps"]):
        ...
        images.append(render_frame(env, new_api, cfg["gray"]))
